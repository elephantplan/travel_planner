...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "day-plan") {
      ...
      // same guard for the day-title renames: only the days actually in scope,
      // and only non-empty strings — a swap that leaves the old theme in the
      // header is exactly the "changes applied but title still says the old
      // day" bug this is here to fix.
      const dayTitlesRaw = parsed.dayTitles && typeof parsed.dayTitles === "object" ? parsed.dayTitles : {};
      const dayTitles: Record<string, string> = {};
      for (const [id, title] of Object.entries(dayTitlesRaw)) {
        if (!dayIdSet.has(id)) continue;
