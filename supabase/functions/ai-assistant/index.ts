...
async function callGemini(apiKeys: string[], prompt: string, opts?: { json?: boolean }): Promise<string> {
  ...
  try {
    if (action === "status") {
      return json({
        ...
        geminiNotice: apiKeys.length ? null : NOT_CONFIGURED_MSG,
      });
    }
    ...
    if (action === "board-ai") {
      ...
      // Every board-ai call is a fresh request with no memory of the last one —
      // same theme, same brief, same itinerary in, so without this the model
      // naturally writes almost the same "analysis" paragraph twice. Handing
      // back its own recent write-ups lets it recognise the repeat and either
      // add a genuinely new angle or just say so briefly instead of restating.
      const priorAnalyses: string[] = (Array.isArray(body?.priorAnalyses) ? body.priorAnalyses : [])
        .map((a: any) => String(a ?? "").trim()).filter(Boolean).slice(0, 3);
      const priorAnalysesBlock = priorAnalyses.length
